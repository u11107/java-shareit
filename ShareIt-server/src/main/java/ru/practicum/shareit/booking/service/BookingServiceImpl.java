...
public class BookingServiceImpl implements BookingService {
    ...
    private void validateUpdateBookingStatus(User user, Item item, Booking booking) {
        if (!user.getId().equals(item.getOwner().getId())) {
            throw new NotFoundException("The user cannot change the booking status");
        }
        if (booking.getStatus().equals(BookingStatus.APPROVED)) {
            throw new ValidationException("Booking status already set to APPROVED");
        }
        if (booking.getStatus().equals(BookingStatus.REJECTED)) {
            throw new ValidationException("Booking status already set to REJECTED");
        }
    }

    private void validateNewBooking(User user, Item item, Booking booking) {
        if (user.getId().equals(item.getOwner().getId())) {
            throw new NotFoundException("The user cannot book his item");
        }
        if (!item.getAvailable()) {
            throw new ValidationException("Item not available for booking");
        }
        if (booking.getEnd().isBefore(booking.getStart())) {
            throw new ValidationException("The end of the booking cannot be before the start of the booking.");
        }
    }
}
